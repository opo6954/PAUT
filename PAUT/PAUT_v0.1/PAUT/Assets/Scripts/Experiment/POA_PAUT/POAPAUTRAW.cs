...
public class POAPAUTRAW : Timeline {
    ...
    void Demo()
    {
        ...
        FEActions.Add(new Action(ActionType.DecreaseFloatVariable, GameObject.Find("Fire").gameObject.GetComponent<Asset>(), "HP", 50.0f));
		...
		List<Action> fireOffActions = new List<Action>();
		fireOffActions.Add(new Action(ActionType.DeactiveRender, GameObject.Find("Fire").gameObject.GetComponent<Asset>()));
		fire.addAssetTrigger(player, GameObject.Find("Fire").gameObject.GetComponent<Asset>(), new MonitorFloatVariableParam(TriggerConditionType.MonitorValueCaptured, "HP", 0, TriggerOperator.SmallerOrEqual), fireOffActions);
