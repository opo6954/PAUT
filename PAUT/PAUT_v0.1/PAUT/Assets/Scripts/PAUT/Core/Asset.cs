...
namespace PAUT
{
    ...
    public class Asset : Placable
    {
        ...
        public class DictionaryFloat
        {
            ...
            public DictionaryFloat(string _key, float _value) { key = _key; value = _value; }
        }
        /*
        ...
        */
        ...
        public class DictionaryBool
        {
            ...
            public DictionaryBool(string _key, bool _value) { key = _key; value = _value; }
        }

        [SerializeField]
        public List<DictionaryFloat> floatVariables;
        [SerializeField]
        public List<DictionaryBool> booleanVariables;
        ...
        private void Awake()
        {
            // if it uses the property window of Unity, uncomment it
            //setupDictionaries();
        ...
        public void setupDictionaries()
        {
            ...
            if (floatVariables == null)
                floatVariables = new List<DictionaryFloat>();
            if (booleanVariables == null)
                booleanVariables = new List<DictionaryBool>();
		...
		private void FixedUpdate(){
			if (targetAssetTriggers == null)
				return;
			foreach (AssetTrigger at in targetAssetTriggers) {
				monitorFloatValue (at);
			}
		}
		...
		*/
		protected void monitorFloatValue(AssetTrigger at)
		{
			if (at.tTriggerParam is MonitorFloatVariableParam && !(at.tTriggerParam is MonitorDistanceParam))
			{
				MonitorFloatVariableParam mfvp = (MonitorFloatVariableParam)at.tTriggerParam;
				if (!dicFloat.ContainsKey(mfvp.key))
				{
					Debug.Log("Event - monitorFloatValue - Not valid key value is detected.");
				}
				float targetValue = dicFloat[mfvp.key];
				//Debug.Log("Event - monitorValue - " + targetValue);
				float threshold = mfvp.value;
				if (((MonitorFloatVariableParam)at.tTriggerParam).tOperator == TriggerOperator.Equal)
				{
					if (targetValue == threshold)
					{
						AssetTrigger e1 = new AssetTrigger(null, this, new MonitorFloatVariableParam(TriggerConditionType.MonitorValueCaptured, true), null);
						Asset.OnEventReceive(e1);
					}
				}
				else if (((MonitorFloatVariableParam)at.tTriggerParam).tOperator == TriggerOperator.Larger)
				{
					if (targetValue > threshold)
					{
						AssetTrigger e1 = new AssetTrigger(null, this, new MonitorFloatVariableParam(TriggerConditionType.MonitorValueCaptured, true), null);
				...
				else if (((MonitorFloatVariableParam)at.tTriggerParam).tOperator == TriggerOperator.LargerOrEqual)
				{
					if (targetValue >= threshold)
					{
						AssetTrigger e1 = new AssetTrigger(null, this, new MonitorFloatVariableParam(TriggerConditionType.MonitorValueCaptured, true), null);
				...
				else if (((MonitorFloatVariableParam)at.tTriggerParam).tOperator == TriggerOperator.Smaller)
				{
					if (targetValue < threshold)
					{
						AssetTrigger e1 = new AssetTrigger(null, this, new MonitorFloatVariableParam(TriggerConditionType.MonitorValueCaptured, true), null);
				...
				else if (((MonitorFloatVariableParam)at.tTriggerParam).tOperator == TriggerOperator.SmallerOrEqual)
				{
					if (targetValue <= threshold)
					{
						//Debug.Log("Event - Trigger Captured");
